  ...
  <header class="pt-4 px-4">
    ...
    <div class="row placeholder-glow">
      <div class="col">
        ...
        <span v-else>Zeige {{ visibleList.length }} von {{ filteredList.length }} Aufträgen ({{ salesOrdersWithDeliveryProblems.length }} insgesamt)</span>
      </div>
...
export default {
  ...
  computed: {
    ...
    salesOrdersWithDeliveryProblems() {
      return this.openSalesOrders.filter((so) => so.SalesOrderPos.some((pos) => pos.deliveryStatus === 5));
    },
    isLoading() {
    ...
    filteredList() {
      return this.salesOrdersWithDeliveryProblems.filter((el) => {
    ...
    availableStatuses() {
      const arr = this.salesOrdersWithDeliveryProblems.map((el) => el.OrderStateDesc);
    ...
    availableUsernames() {
      const arr = this.salesOrdersWithDeliveryProblems.map((el) => el.Username);
      const set = [...new Set(arr)];
      const sorted = set.sort((a, b) => a.localeCompare(b));
  ...
  created() {
    console.log('DeliveryProblems: created');
