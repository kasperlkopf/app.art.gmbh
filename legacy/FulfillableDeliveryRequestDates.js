// FulfillableDeliveryRequestDates.js
...
import SalesOrderCard from '/components/SalesOrderCard.js';

const template = `
  <!-- header -->
  <header class="pt-4 px-4">
    <h3>Erfüllbare Wunschtermine</h3>

    <div class="row placeholder-glow">
      <div class="col">
        <span class="placeholder rounded opacity-25" style="width: 290px" v-if="isLoading"></span>
        <span v-else>Zeige {{ visibleList.length }} von {{ filteredList.length }} Aufträgen ({{ salesOrdersWithFulfillableDeliveryRequestDates.length }} insgesamt)</span>
      </div>
    ...
    <div class="row flex-nowrap">
      ...
      <div class="col-auto pe-0">
        <div class="btn-group">
          ...
          <MultiSelect :items="availableStatuses" v-model:selected="selectedStatuses" label="Belegstatus">
            <template #body>
          ...
          <MultiSelect :items="availableSnoozeStates" v-model:selected="selectedSnoozeStates" label="Snoozestatus" buttonClass="rounded-end">
            <template #body>
      ...
      <div class="col-auto">
        <select class="form-select" v-model="sorting">
          ...
          <option value="6" selected>KD#: aufsteigend</option>
          <option value="7">KD#: absteigend</option>
        </select>
      </div>
    </div>
  </nav>
  ...
  <div class="px-4">
    ...
    <template v-if="visibleList.length && !isLoading">
      <sales-order-card v-for="item in visibleList" :salesOrder="item" :key="item.OrderID" />
...
export default {
  name: 'FulfillableDeliveryRequestDates',
  ...
  data() {
    return {
      ...
      selectedUsernames: [],
      selectedStatuses: [],
      selectedSnoozeStates: ['Ungesnoozt', 'Überfällig'],
      availableSnoozeStates: ['Ungesnoozt', 'Gesnoozt', 'Überfällig'],
      sorting: '0',
      interval: '',
      timeSinceUpdated: '',
      currentPage: 1,
  ...
  computed: {
    openSalesOrders() {
      return this.$store.state.openDocuments.documents.openSalesOrders;
    },
    salesOrdersWithFulfillableDeliveryRequestDates() {
      return this.openSalesOrders.filter((so) => so.SalesOrderPos.some((pos) => pos.deliveryStatus === 3));
    },
    isLoading() {
    ...
    filteredList() {
      return this.salesOrdersWithFulfillableDeliveryRequestDates.filter((el) => {
        ...
        const hasSnoozeState = !this.selectedSnoozeStates.length || (this.selectedSnoozeStates.includes('Ungesnoozt') && el.SnoozedUntilDate === '') || (this.selectedSnoozeStates.includes('Gesnoozt') && new Date(el.SnoozedUntilDate) > new Date()) || (this.selectedSnoozeStates.includes('Überfällig') && new Date(el.SnoozedUntilDate) <= new Date());

        return str.includes(this.search.toLowerCase()) && hasUsername && hasStatus && hasSnoozeState;
      });
    },
    sortedList() {
    ...
    availableStatuses() {
      const arr = this.salesOrdersWithFulfillableDeliveryRequestDates.map((el) => el.OrderStateDesc);
    ...
    availableUsernames() {
      const arr = this.salesOrdersWithFulfillableDeliveryRequestDates.map((el) => el.Username);
      const set = [...new Set(arr)];
      const sorted = set.sort((a, b) => a.localeCompare(b));
  ...
  methods: {
    makeFilterString(el) {
      const arr = [el.OrderNumber, el.Matchcode, el.CustomerNumber, el.ContactName, el.ReferenceText, el.Username, el.OrderStateDesc, el.MemoText];

      el.SalesOrderPos.forEach((sop) => arr.push(...[sop.ProductNumber, sop.Name1, sop.Name2, sop.ReferenceNumber, sop.Quantity]));
  ...
  created() {
    console.log('FulfillableDeliveryRequestDates: created');

    if (!this.openSalesOrders.length) {
      this.$store.dispatch('getOpenDocuments');
    }
  },
  activated() {
